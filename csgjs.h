...
csgjs_vector unit(const csgjs_vector & a);
csgjs_vector cross(const csgjs_vector & a, const csgjs_vector & b);
...
struct csgjs_vector2 {
    ...
    explicit csgjs_vector2(float x, float y) : x(x), y(y) {
    }
};
...
csgjs_model csgsmodel_cube(const csgjs_vector &center = { 0.0f, 0.0f, 0.0f }, const csgjs_vector &dim = { 1.0f, 1.0f,1.0f });
...
csgjs_vector operator - (const csgjs_vector & a, const csgjs_vector & b) { return csgjs_vector(a.x - b.x, a.y - b.y, a.z - b.z); }
...
inline static csgjs_vector2 lerp(const csgjs_vector2 & a, const csgjs_vector2 & b, float v) { return csgjs_vector2{ a.x + (b.x - a.x) * v, a.y + (b.y - a.y) * v }; }
...
csgjs_vector unit(const csgjs_vector & a) { return a / length(a); }
csgjs_vector cross(const csgjs_vector & a, const csgjs_vector & b) { return csgjs_vector(a.y * b.z - a.z * b.y, a.z * b.x - a.x * b.z, a.x * b.y - a.y * b.x); }
...
std::vector<csgjs_polygon> csgjs_operation(const std::vector<csgjs_polygon> &apoly, const std::vector<csgjs_polygon> &bpoly, csg_function fun) {
...
inline static std::vector<csgjs_polygon> csgjs_operation(const csgjs_model & a, const csgjs_model & b, csg_function fun)
{
...
csgjs_model csgsmodel_cube(const csgjs_vector &center, const csgjs_vector &dim) {
	...
	for (const auto &q : quads) {
		...
		for (auto i : q.indices) {
			csgjs_vector pos(
				center.x + dim.x * (2.0f * !!(i & 1) - 1),
				center.y + dim.y * (2.0f * !!(i & 2) - 1),
